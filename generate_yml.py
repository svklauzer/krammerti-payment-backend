...
# --- ФИНАЛЬНЫЙ СЛОВАРЬ ВАЛЮТ (основан на анализе прайс-листа) ---
# Он содержит ВСЕ варианты, которые встречаются в файле.
# Ключи приведены к верхнему регистру для надежного сопоставления.
CURRENCY_MAP = {
    'РУБ.': 'RUR',
    'USD': 'USD',
    'У.Е.': 'USD', # По соглашению 1С, у.е. (условные единицы) эквивалентны USD
    'KZT': 'KZT',
    'BYN': 'BYN',
    'KGS': 'KGS',
    'EUR': 'EUR',
...
def download_and_extract_pricelist(url):
    """Скачивает и читает прайс-лист в DataFrame."""
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            if not xls_filename:
                raise FileNotFoundError("В архиве не найден .xls файл.")
...
def parse_pricelist(df):
    """Парсит DataFrame, извлекая категории, товары и используемые валюты."""
    print("Обработка данных...")
    
    # Надежное переименование колонок по их позиции
    df.rename(columns={
        df.columns[0]: 'id',
        df.columns[1]: 'name',
        df.columns[3]: 'currency',
        df.columns[4]: 'price'
    }, inplace=True)
    ...
    used_currencies = {'RUR'} # RUR всегда должен быть в списке
    
    category_regex = re.compile(r'Раздел\s+(\d+)', re.IGNORECASE)

    for index, row in df.iterrows():
        # Определяем основной текст строки (может быть в колонке id или name)
        ...
        match = category_regex.search(text_to_check)
        ...
        # Если это товар
            ...
            # Обрабатываем цену, корректно работая с запятой
            ...
            # Проверяем, что у товара есть все необходимые атрибуты
            if pd.notna(price) and price > 0 and pd.notna(product_id) and product_name != 'nan':
                # --- ЛОГИКА ОПРЕДЕЛЕНИЯ ВАЛЮТЫ ДЛЯ КАЖДОЙ СТРОКИ ---
                currency_from_file = str(row['currency']).strip().upper()
                
                # Ищем значение из файла в нашем словаре. Если не найдено, будет None.
                currency_id = CURRENCY_MAP.get(currency_from_file)
                
                # Если валюта не нашлась или пустая, по умолчанию RUR
                if not currency_id:
                    currency_id = 'RUR'
                ...
                offers.append({
                    ...
                    'name': product_name,
                    'price': price,
                    'currencyId': currency_id, # Устанавливается индивидуально для товара
                    'categoryId': current_category_id,
                    'url': f"{SHOP_URL}/product/{product_id_str}",
                    'picture': f"{SHOP_URL}/images/placeholder.jpg"
                })
...
def generate_yml_feed(categories, offers, used_currencies):
    """Генерирует YML файл на основе полученных данных."""
    ...
    # Создаем блок валют на основе ВСЕХ найденных в прайсе
    ...
    for offer in offers:
        ...
        ET.SubElement(offer_element, 'categoryId').text = offer['categoryId']
