...
def parse_pricelist(df):
    ...
    df.rename(columns={
        df.columns[0]: 'id', df.columns[1]: 'name',
        df.columns[3]: 'currency', df.columns[4]: 'price'
    }, inplace=True)
    ...
    # --- КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Добавлен якорь '^' в начало regex ---
    # Теперь он ищет "Раздел" ТОЛЬКО в начале строки.
    ...
    for index, row in df.iterrows():
        ...
        # Используем .match(), который ищет только в начале строки. Это надежнее.
